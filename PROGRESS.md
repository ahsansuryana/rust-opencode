...
| 6 | oc-tool (bag. 2: shell/search/web) | in-progress | 2026-08-23 | 6a DONE: shell_detect (meta/args/login-scripts), ShellTool run (ring-buffer, overflow->truncation dir, tail, timeout+<shell_metadata>), permission scan (tokenizer; tree-sitter deviasi tercatat). 6b sisa: webfetch/websearch tools, ShellPrompt render ${key} penuh, apply_patch tool, cygpath resolution |
