...
| 7 | oc-provider | in-progress | 2026-08-23 | 7a DONE: tipe Model/Info/ListResult, sort/parse/default_model_ids, error classes (pesan persis), ProviderAuth service (authorize/callback -> oc-auth). 7b DONE: transform_messages.rs (normalize/sanitize/scrub), transform.rs (sampling temp/topP/topK, openai efforts tiers, anthropic/google adaptive thinking, schema sanitizers openai/moonshot/gemini) — 18 test. 7c sisa: applyCaching/unsupportedParts/message() pipeline + variants()/options()/provider_options() + HTTP client Anthropic/OpenAI/Google SSE | HTTP client Anthropic/OpenAI/Google + SSE streaming |
